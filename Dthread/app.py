...
# ============================================================
    ...
    if ideas:
        ...
        for idea in filtered:
            links_html = '<div class="idea-link">🔗 ' + ' · '.join(idea["links"]) + '</div>' if idea.get("links") else ""
            st.markdown(
                f'<div class="idea-card">'
                f'<div style="font-family:Bebas Neue,sans-serif;font-size:1.1rem;letter-spacing:2px;color:#4488ff;">{idea["title"].upper()}</div>'
                f'<div style="font-family:DM Sans,sans-serif;font-size:0.82rem;color:#888;margin-top:0.4rem;line-height:1.6;">{idea["body"]}</div>'
                + links_html +
                f'<div style="font-family:DM Mono,monospace;font-size:0.6rem;color:#555;margin-top:0.5rem;">Tags: {idea.get("tags","—")} · {idea["date"]}</div></div>',
                unsafe_allow_html=True)
